...
namespace My
{
    ...
    inline std::string formatJson(std::string json)
    {
        std::string result = "";
        int level = 0;
     ...
     */
    class MyJson;
    typedef ValueArray<MyJson> xarray;
    typedef ValueArray<MyJson> xobject;

    class MyJson
    {
        friend class ValueArray<MyJson>;
    ...
    template <>
    inline xarray MyJson::Get(std::string key)
    {
    ...
    inline std::ostream &operator<<(std::ostream &os, MyJson &ob)
    {
        os << ob.WriteJson();
        return os;
    }

    inline std::string MyJson::WriteJson(int type)
    {
    ...
    template <>
    inline void Value::Set(MyJson v)
    {
        std::ostringstream oss;
        if (v.sub_type_ == 1)
        {
